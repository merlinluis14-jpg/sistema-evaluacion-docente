...
| Gestion academica | El administrador puede gestionar carreras, docentes, alumnos, grupos, materias y periodos. | `src/app/admin/*` |
...
| Exportacion de resultados | Se generan `PDF alumnos`, `PDF institucional` y `Excel` desde la vista de reportes. | `src/app/admin/reportes/ExportButtons.tsx` |
...
| Bitacora administrativa | Toda accion administrativa relevante queda registrada en `AdminLog`. | `src/lib/adminLog.ts`, `src/app/admin/logs/page.tsx` |
...
| Requisito | Implementacion actual | Evidencia principal |
| --- | --- | --- |
| Proteccion de credenciales | Las contrasenas se almacenan con hash bcrypt. | `src/lib/auth.ts`, `src/app/admin/administradores/actions.ts`, importadores CSV |
...
| Requisito | Implementacion actual | Evidencia principal |
| --- | --- | --- |
| Respaldo diario | Se incluyen scripts para respaldo de PostgreSQL en Windows y Linux/macOS. | `scripts/backup.bat`, `scripts/backup.sh` |
...
| Rendimiento de consultas | El esquema Prisma incluye indices para reportes, docentes, materias y evaluaciones. | `prisma/schema.prisma`, migraciones Prisma |
